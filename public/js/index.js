$(document).ready(function() {
  ...
  function renderEmpty() {
    const emptyAlert = $(
      "<div class='alert'><h2>No articles found.</h2><div><h3>What Would You Like To Do?</h3></div><div><a class='scrape'><h4>Try Scraping New Articles</h4></a><a href='/saved'><h4>Go to Saved Articles</h4></a></div></div>"
    );
  ...
  function renderEmptySaved() {
    const emptyAlert = $(
      "<div class='alert'><h2>No articles found.</h2><div><h3>What Would You Like To Do?</h3></div><div><a href='/'><h4>Go Home</h4></a></div></div>"
    );
  ...
  function createUnsavedCard(article) {
    const newCard = $(
      `<div class='article'><div style="background-image: url(${
        article.imageLink
      }); background-size: cover; background-position: center;"></div><div><a href="https://www.newyorker.com${
        article.link
      }"><h1>${article.title}</h1></a><p>${
        article.summary
      }</p></div><div><button class='save' data-id='${
        article._id
  ...
  function createSavedCard(article) {
    ...
    newCard.append(
      `<div style="background-image: url(${
        ...
        article.summary
      }</p></div><div><button class="attach-note" data-id="${
        article._id
      }">Attach Note</button><button class="remove-save" data-id="${
        article._id
      }">Remove From Saved</button><button class="show-notes" data-id="${
        article._id
      }">Show Notes</button></div><div class="modal" style="display: none"><form action=""><input class="name" type="text" placeholder="Name" required autofocus><textarea class="message" cols="30" rows="5" placeholder="Leave a note" required></textarea><button data-id="${
        article._id
      }" class="note-submit">Submit</button></form></div>`
    );
    let noteModal = $(
      "<div class='modal-notes' style='display: none'><h2>Notes</h2></div>"
    );
    if (notes && notes.length) {
      ...
      for (i = 0; i < notes.length; i++) {
        let newDiv2 = $("<div>");
        let newNote = $(
          `<span>${notes[i].name}: </span><span> "${notes[i].message}" </span>`
        );
        let deleteButton = $(
          `<button class='delete-note' data-id='${notes[i]._id}'></button>`
        ).text("Delete");
